...
const MyDataProductSelect = ({
  ...
  const selectedProductTitle = formatMessage({
    id: 'myData_selected_product_title',
  });
  ...
  return (
    <div className="iam-mydata-content-container">
      <div className="iam-mydata-flex-container">
        <div className="iam-mydata-intuit-products">{selectedProductTitle}</div>
        {displayIcon === CHECK_BOX && (
          <div className="iam-mydata-flex-link">
            <Link onClick={onSelectAllProducts}>{selectAllButton}</Link>
          </div>
        )}
      </div>
      <ul>
        {productList
          .filter((product) => {
            return !deleteSelected && !downloadSelected
              ? true
              : (deleteSelected && product.deleteSelected) ||
                ...
                <li className="iam-mydata-list-item-flex-container">
                  ...
                  {displayIcon === CHEVRON_ICON && (
                    <span className="iam-mydata-action-column">
                      <button
                        className="iam-mydata-button-nostyling"
                        onClick={onChevronSelection}
                      >
                        <i
                          className="iam-mydata-chevron"
                  ...
                  {displayIcon === CHECK_BOX && (
                    <span className="iam-mydata-action-column">
                      <Checkbox
                        ...
                        checked={
                          (flow.match('DELETE') && product.deleteSelected) ||
                          (flow.match('DOWNLOAD') && product.downloadSelected)
                        }
